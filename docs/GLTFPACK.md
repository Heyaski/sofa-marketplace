...
## Цель: 60 MB → ~10–15 MB (загрузка 7–10 сек)

По умолчанию `-si 0.2` (упрощение до ~20% полигонов) — даёт ~10–15 MB и загрузку 7–10 сек. Для меньшего сжатия: `GLB_SI_RATIO=0.33` в `.env` → ~30 MB, 30 сек. Режим добавляет mesh optimization + quantization (KHR_mesh_quantization), совместим с model-viewer.
...
| `-si 0.2` | По умолчанию — ~10–15 MB, загрузка 7–10 сек |
| `-si 0.33` | Меньше сжатие — ~30 MB, 30 сек (GLB_SI_RATIO=0.33) |
| `-si 0.5` | Минимальное сжатие — ~35 MB (если артефакты) |
| `-cc` | Доп. сжатие — требует meshopt decoder в приложении, не используется |

**Настройка:** `.env` — `GLB_SI_RATIO=0.2` (по умолчанию), `GLB_SI_RATIO=0.33` для лучшего качества. `GLB_SI_RATIO_LARGE=0.25` — для файлов > 40 MB.
