...
## Цель: все файлы ≤ 10 MB

По умолчанию целевой размер 10 MB. gltfpack итеративно подбирает `-si` (0.2 → 0.15 → 0.12 → 0.1 → 0.08) до достижения размера. При импорте и в `optimize_glb_s3` — автоматически. Настройка: `GLB_TARGET_MB=10` в `.env`.
