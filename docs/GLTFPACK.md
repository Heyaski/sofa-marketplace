...
**Восстановление качества** (если модели стали угловатыми): нужны оригинальные файлы. Скрипт ищет в папках (по порядку): `backups/glb-assets-original`, `media/assets-backup`, `media/assets`. Проверить: `python manage.py optimize_glb_s3 --list-backup`. Восстановить: `GLB_TARGET_MB=30` в `.env`, затем `python manage.py optimize_glb_s3 --restore-backup`.
